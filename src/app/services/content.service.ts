...
export class ContentService {
  ...
  async getContent(type: string): Promise<IContent[]> {
    ...
    const statement = query(collections, where('tipo', "==", type));
  ...
  async addToFavorites(id: string): Promise<void> {
    ...
    if (docSnapshot.exists()) {
      const currentFavoriteValue = docSnapshot.data() as IContent;

      await updateDoc(contentDocRef, { favorito: !currentFavoriteValue.favorito });
    } else {
